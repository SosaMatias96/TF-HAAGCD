"""
Creador de base de datos con temperatura ambiente actual
...
for i in range(len(lista_archivos)):
    ...
    M_Tc_A        = np.hstack((t,x_Tc_A,y_Tc_A,To_v,q_v,T_amb,Tc_A))
    M_Tc_M        = np.hstack((t,x_Tc_M,y_Tc_M,To_v,q_v,T_amb,Tc_M))
    M_Tcal_int_M  = np.hstack((t,x_Tcal_int_M,y_Tcal_int_M,To_v,q_v,T_amb,Tcal_int_M))
    M_Top_int_M   = np.hstack((t,x_Top_int_M,y_Top_int_M,To_v,q_v,T_amb,Top_int_M))
    M_Tcal_int_A  = np.hstack((t,x_Tcal_int_A,y_Tcal_int_A,To_v,q_v,T_amb,Tcal_int_A))
    M_Tcal_int_B  = np.hstack((t,x_Tcal_int_B,y_Tcal_int_B,To_v,q_v,T_amb,Tcal_int_B))
    ...
    if lista_archivos[i][14]=='O':
        ...
        M_Tc_B = np.hstack((t,x_Tc_B,y_Tc_B,To_v,q_v,T_amb,Tc_B))
        M_matriz = np.vstack((M_matriz,M_Tc_B))
    DATOS = np.vstack((DATOS,M_matriz))
    del df,datos
    
#%% guardo nuevo archivos CSV
encab = ["t","x","y","To","q","T_amb","T"]
df_N = pd.DataFrame(DATOS, columns= encab)
df_N.to_csv("DATOS_con_Tamb.csv", index=False, encoding='utf-8')
